...
export default function Home({ allPostsData }) {
  ...
  const articlesRef = useRef(null); // 文章区域引用
  ...
  const [isArticlesVisible, setIsArticlesVisible] = useState(false); // 文章显隐状态
  ...
  return (
    <div className={`min-h-screen selection:bg-blue-600 selection:text-white transition-colors duration-700 ${isDarkMode ? 'dark bg-black text-white' : 'bg-[#fafafa] text-black'}`}>
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-black/5 dark:border-white/10 bg-white/80 dark:bg-black/80 backdrop-blur-xl">
        <div className="max-w-[1440px] mx-auto px-6 md:px-10 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-8 text-[11px] font-bold uppercase tracking-[0.2em]">
            ...
            <button onClick={toggleDarkMode} className="text-lg w-8 h-8 flex items-center justify-center rounded-full border border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5 transition-all focus:outline-none">
              {isDarkMode ? '☼' : '☾'}
            </button>
          </div>

          <div className="flex md:hidden items-center space-x-4 z-50">
            <button onClick={() => setIsSearchOpen(true)} className="p-2 opacity-60 focus:outline-none"><SearchIcon /></button>
        ...
        {/* 移动端菜单 */}
        <div className={`fixed inset-0 bg-white/95 dark:bg-black/95 backdrop-blur-2xl transition-all duration-500 md:hidden z-40 ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
          <div className="flex flex-col items-center justify-center h-full space-y-8 text-2xl font-black tracking-tighter uppercase">
            <Link href="/archive"><a onClick={closeMobileMenu}>Archive</a></Link>
            <Link href="/tags"><a onClick={closeMobileMenu}>Tags</a></Link>
            <Link href="/about"><a onClick={closeMobileMenu}>About</a></Link>
            <button onClick={() => { toggleDarkMode(); closeMobileMenu(); }} className="text-sm border border-black/10 dark:border-white/10 px-6 py-2 rounded-full font-bold uppercase tracking-widest mt-4">
      ...
      <main className="relative z-10 max-w-[1440px] mx-auto px-6 md:px-10 pt-48 pb-32">
        {/* Hero Section - 增加了底部边距以确保隔离 */}
        <header className="min-h-[60vh] flex flex-col justify-center mb-32 md:mb-64">
          <div className={`transition-all duration-[1500ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${showHero ? 'translate-y-0 opacity-100' : 'translate-y-16 opacity-0'}`}>
            <h1 className="text-[clamp(3.5rem,11vw,10rem)] leading-[0.82] font-black tracking-tighter mb-12">
              BEYOND <br />
  ...
  <Link href={`/posts/${post.slug}`}>
    <a className="block h-full relative p-10 flex flex-col justify-end group">
      ...
      <div className="relative z-10">
        ...
        {featured && (
          <p className="mt-10 text-sm opacity-0 group-hover:opacity-60 transition-all duration-700 translate-y-6 group-hover:translate-y-0 line-clamp-2 max-w-xl font-medium leading-relaxed hidden sm:block">
