...
const POSTS_PER_PAGE = 10; // 设置为10篇文章一页
...
export default function Home({ allPostsData }) {
  ...
  return (
    <div className={`min-h-screen selection:bg-blue-600 selection:text-white transition-colors duration-700 ${isDarkMode ? 'dark bg-black text-white' : 'bg-[#fafafa] text-black'}`}>
      <Head>
        <title>TYPACE — Minimalist Space</title>
        <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;700;900&display=swap" rel="stylesheet" />
      </Head>

      <canvas ref={canvasRef} className="fixed inset-0 pointer-events-none z-0 overflow-hidden" />
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-black/5 dark:border-white/10 bg-white/80 dark:bg-black/80 backdrop-blur-xl">
        <div className="max-w-[1440px] mx-auto px-10 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-8 text-[11px] font-bold uppercase tracking-[0.2em]">
            ...
            <button onClick={toggleDarkMode} className="text-lg w-6 h-6 flex items-center justify-center rounded-full border border-black/10 dark:border-white/10">
              {isDarkMode ? '☼' : '☾'}
            </button>
          </div>
        </div>
        ...
        <header className="mb-40">
          <h1 className="text-[clamp(3rem,12vw,10rem)] leading-[0.8] font-black tracking-tighter mb-12">
            ...
            <div className="relative h-[1.1em] overflow-hidden">
              <div 
                className="transition-transform duration-[1000ms] ease-[cubic-bezier(0.85,0,0.15,1)]"
          ...
          <p className="max-w-xl text-base font-medium opacity-40 leading-relaxed italic">
            "{displayText}"<span className="inline-block w-1.5 h-4 bg-blue-500 ml-1 animate-pulse" />
          </p>
        </header>

        {/* Vercel Bento Grid (10篇文章排列) */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          {paginatedPosts.map((post, idx) => {
            // 定义Bento Grid的布局规律：8-4, 4-4-4, 4-8...
            const gridClass = [0, 5, 8].includes(idx) ? 'md:col-span-8' : 'md:col-span-4';
            return (
              <div key={post.slug} className={`${gridClass} min-h-[400px] border border-black/5 dark:border-white/10 hover:border-blue-500/50 transition-all duration-500 group relative bg-white dark:bg-[#050505]`}>
      ...
      <footer className="max-w-[1440px] mx-auto px-10 py-20 opacity-30 text-[9px] tracking-[0.4em] uppercase flex justify-between items-center">
        <span>© TYPACE DIGITAL SYSTEM</span>
        <div className="h-px w-20 bg-current mx-4 hidden md:block"></div>
...
const ArticleBox = ({ post, featured }) => (
  <Link href={`/posts/${post.slug}`}>
    <a className="block h-full relative p-8 flex flex-col justify-end overflow-hidden group">
      {/* 减弱后的灰度图片：初始 grayscale-30, 悬停恢复 */}
      <div className="absolute inset-0 z-0">
        <img 
          src={post.cover || 'https://images.unsplash.com/photo-1614850523296-d8c1af93d400?q=80&w=2070&auto=format&fit=crop'} 
          className="w-full h-full object-cover grayscale-[0.3] opacity-10 group-hover:grayscale-0 group-hover:opacity-40 group-hover:scale-110 transition-all duration-1000 ease-out"
          alt=""
        />
      </div>
      ...
      <div className="relative z-10">
        ...
        {featured && (
          <p className="mt-8 text-sm opacity-0 group-hover:opacity-50 transition-all duration-700 translate-y-4 group-hover:translate-y-0 line-clamp-2 max-w-lg font-medium leading-relaxed">
            {post.excerpt || "Dive into the technical intricacies and design philosophies of this latest architectural exploration..."}
          </p>
        )}
      </div>
