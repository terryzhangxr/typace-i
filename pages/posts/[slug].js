...
export default function Post({ frontmatter, contentHtml, recommendedPosts, allPostsData }) {
  ...
  return (
    <div className={`min-h-screen selection:bg-blue-600 selection:text-white transition-colors duration-700 ${isDarkMode ? 'dark bg-black text-white' : 'bg-[#fafafa] text-black'}`}>
      ...
      {/* 图片预览 */}
      {previewImage && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-white/80 dark:bg-black/90 backdrop-blur-3xl p-4 md:p-20 cursor-zoom-out animate-in fade-in duration-300" onClick={() => setPreviewImage(null)}>
          <div className="relative animate-in zoom-in-95 duration-300 ease-out">
            <img src={previewImage} className="max-w-full max-h-[90vh] object-contain shadow-2xl border border-black/5 dark:border-white/10 rounded-sm" alt="Preview" />
          </div>
        </div>
      )}
      ...
      <style jsx global>{`
        ...
        .prose-terminal h1, .prose-terminal h2, .prose-terminal h3 {
          scroll-margin-top: 100px; /* 预留导航栏位置 */
        }
        ...
        .prose-terminal img:hover { transform: scale(1.02); }
